...
def _score_population_impact(ctx: IncidentContext, cfg: PriorityConfig) -> FactorScore:
    pop = ctx.people_at_risk if ctx.people_at_risk is not None else ctx.affected_population
    if pop is None and ctx.incident and ctx.incident.people_at_risk is not None:
        pop = ctx.incident.people_at_risk

    trapped = ctx.people_trapped
    if (trapped is None or trapped == 0) and ctx.incident and ctx.incident.people_trapped:
        trapped = ctx.incident.people_trapped

    if (pop is None or pop == 0) and not trapped:
        return FactorScore(
    ...
    base_pop = pop or 0
    raw_norm = min(base_pop / cfg.population_norm, 1.0)
    
    # Trapped individuals carry intense life-safety weight
    if trapped and trapped > 0:
        # Trapped multiplier elevates priority band
        trapped_norm = min(0.40 + (trapped / 20.0) * 0.60, 1.0)
        normalised = round(max(raw_norm, trapped_norm), 6)
    else:
        normalised = round(raw_norm, 6)

    codes_list: list[str] = []
    if trapped and trapped > 0:
        codes_list.append("PEOPLE_TRAPPED")
    if normalised >= 0.5:
        codes_list.append(RC_POPULATION_LARGE)
    elif normalised >= 0.10:
        codes_list.append(RC_POPULATION_MODERATE)

    return FactorScore(
        name="population_impact",
        raw_value=pop if pop is not None else (trapped if trapped else 0),
        normalised=normalised,
        weight=cfg.weight_population_impact,
        contribution=round(normalised * cfg.weight_population_impact, 6),
        reason_codes=tuple(codes_list),
    )
...
class IncidentPriorityEngine:
    ...
    def explain_drivers(
        self,
        context: IncidentContext,
        result: PriorityResult | None = None,
    ) -> list[str]:
        """Produce human-readable life-safety and operational driver strings."""
        if result is None:
            result = self.score(context)

        drivers: list[str] = []

        # 1. Trapped people (Highest Life Safety)
        trapped = context.people_trapped
        if (
            (trapped is None or trapped == 0)
            and context.incident
            and context.incident.people_trapped
        ):
            trapped = context.incident.people_trapped
        if trapped and trapped > 0:
            drivers.append(f"{trapped} people trapped requiring immediate life-safety rescue")

        # 2. People at risk
        pop = context.people_at_risk or context.affected_population
        if pop is None and context.incident and context.incident.people_at_risk is not None:
            pop = context.incident.people_at_risk
        if pop and pop > 0:
            drivers.append(f"{pop} people at direct flood risk")

        # 3. Critical Facilities
        if context.critical_facility_count > 0:
            fac_str = f"{context.critical_facility_count} critical facilities at risk"
            if context.incident and context.incident.critical_facility:
                fac_str += f" ({context.incident.critical_facility})"
            drivers.append(fac_str)
        elif context.incident and context.incident.critical_facility:
            drivers.append(f"Critical facility at risk: {context.incident.critical_facility}")

        # 4. Road / Access Disruption
        if context.road_blocked:
            drivers.append("Primary road network blocked / severely degraded emergency access")

        # 5. Response Deadline
        if context.hours_until_deadline is not None:
            if context.hours_until_deadline <= 2.0:
                drivers.append(
                    f"Urgent response deadline ({context.hours_until_deadline:.1f} hrs remaining)"
                )
            else:
                drivers.append(
                    f"Response deadline window ({context.hours_until_deadline:.1f} hours)"
                )

        # 6. Infrastructure Dependencies
        if context.infra_dependency_count > 0:
            drivers.append(
                f"{context.infra_dependency_count} shared infrastructure assets at cascade risk"
            )

        # 7. Severity Level fallback driver
        if not drivers or context.incident.severity in (SeverityLevel.CRITICAL, SeverityLevel.HIGH):
            drivers.append(f"Classified severity: {context.incident.severity.upper()}")
