...
class IncidentContext(BaseModel):
    ...
    people_at_risk: int | None = Field(
        default=None,
        ge=0,
        description="Explicit count of people at risk (mirrors affected_population if unset).",
    )
    people_trapped: int = Field(
        default=0,
        ge=0,
        description="Count of individuals trapped requiring immediate rescue intervention.",
    )
