...
class Incident(BaseModel):
    ...
    location: str | None = Field(
        default=None,
        description="Exact, approximate, or textual location of the incident.",
    )
    affected_areas: list[str] = Field(
        default_factory=list,
        description="Named areas or sub-zones affected by this incident.",
    )
    people_at_risk: int | None = Field(
        default=None,
        ge=0,
        description="Estimated count of people at risk.",
    )
    people_trapped: int | None = Field(
        default=None,
        ge=0,
        description="Count of people trapped requiring swift life-safety rescue.",
    )
    critical_facility: str | None = Field(
        default=None,
        description="Name or type of critical facility impacted (e.g. hospital).",
    )
    dependencies: list[str] = Field(
        default_factory=list,
        description="IDs of dependent incidents, cascading risks, or critical assets.",
    )
    source: str = Field(
        default="unknown",
        description="Origin source of the incident (e.g. citizen, sensor, grok, detector).",
    )
