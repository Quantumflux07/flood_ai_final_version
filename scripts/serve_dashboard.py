...
Serves V2 REST API endpoints and dashboard static assets.
"""
    ...
    def do_POST(self):
        ...
        if path == "/api/ingest":
            ...
            if not report_text or not str(report_text).strip():
                self._send_json(200, {"success": False, "errors": ["Report text is required."]})
                return

            try:
                res = manager.execute_input(report_text, zone_id_hint)
                if res.get("status") in ("clarify", "rejected", "error"):
                    err_msg = res.get("reason") or res.get("message") or "Input not accepted."
                    self._send_json(200, {
                        "success": False,
                        "errors": [err_msg],
                ...
                traceback.print_exc()
                self._send_json(500, {"success": False, "errors": [str(e)]})
            return
